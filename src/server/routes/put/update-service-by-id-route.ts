...
async function verifyIfAllEmployeesExists(employees: string[]): Promise<boolean> {
	...
	if (employeesMatched.length < employees.length || employees.length < 1) return false
...
export const updateServiceByIdRoute: FastifyPluginAsyncZod = async app => {
	app.put(
		...
		async ({ body, params }) => {
			...
			if (body.employees) {
				if (serviceExists.id && (await verifyIfAllEmployeesExists(body.employees))) {
